...
class DirectionMetric(torch.nn.Module):

    def __init__(self, loss_type='cosine'):
        super(DirectionMetric, self).__init__()
...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., patch_loss_type='mae', direction_loss_type='cosine', clip_model='ViT-L/14'):
        ...
        self.preprocess = transforms.Compose([
            ...
            transforms.CenterCrop(224),           # Crop to ensure the image is exactly 224x224
        ...
        self.patch_loss     = DirectionMetric(patch_loss_type)
        self.direction_loss = DirectionMetric(direction_loss_type)
    ...
    def encode_images(self, images: torch.Tensor) -> torch.Tensor:
        ...
        if images.ndimension() == 3:  # or use len(images.shape) == 3
            # Add a batch dimension, so shape becomes [1, 3, H, W]
    ...
    def forward(self, src_img: Image, source_class: str, target_img: Image, target_class: str, texture_image: torch.Tensor = None):
        clip_loss = 0.0
