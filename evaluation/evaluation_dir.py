...
def calculate_metric(metrics_calculator,metric, src_image, tgt_image, src_mask, tgt_mask,src_prompt,tgt_prompt):
    ...
    if metric=="local_clip":
        return metrics_calculator.compute_local_clip(src_image, src_prompt, tgt_image, tgt_prompt)
    
all_tgt_image_folders={
    "your_method" : "directory_of_images",
    
    # For example:
    # "h_edit_P": "~/results_h_edit_P/",
...
if __name__=="__main__":
    ...
    parser.add_argument('--annotation_mapping_file', type=str, default="./PIE_Bench_Data/mapping_file.json")
    ...
    parser.add_argument('--src_image_folder', type=str, default="/home/yanli/h-edit/text-guided/results/recons")
    parser.add_argument('--res_image_folder', type=str, default="/home/yanli/h-edit/text-guided/results/h_edit")
    parser.add_argument('--tgt_methods', nargs = '+', type=str, default=[
                                                            "h-edit",
                                                            ])
    
    parser.add_argument('--result_path', type=str, default="./results/results.csv")
    ...
    tar_image_list = os.listdir(args.res_image_folder)
    src_image_list = os.listdir(args.src_image_folder)
    tar_image_list.sort()
    src_image_list.sort()
    ...
    for i, (src_image_name, tar_image_name) in enumerate(zip(src_image_list, tar_image_list)):
        
        src_image_path=os.path.join(src_image_folder, src_image_name)
        src_image = Image.open(src_image_path)
        tar_image_path=os.path.join(args.res_image_folder, tar_image_name)
        tar_image = Image.open(tar_image_path)
        evaluation_result=[src_image_name]
